...
- `runectx validate [--json] [--non-interactive] [--explain] [--ssh-allowed-signers PATH] [path]`
  ...
  - emits stable line-oriented `key=value` output for success, validation
    failure, and usage errors, plus a shared `--json` envelope for
    machine-facing automation

## Output Contract

The CLI now supports two machine-facing output modes.
...
`--explain` is accepted across the current alpha.5 command surface, but richer
explanation payloads are still incremental. Current commands emit an
`explain_warning` field when detailed explain data is not yet available.
...
The CLI remains pre-MVP and intentionally scoped. Broader command coverage such
as `bundle resolve`, `doctor`, `init`, and explicit promotion workflows is
